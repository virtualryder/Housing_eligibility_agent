...
const body = [
  ...
  table(["Due process / 800-53 area", "How the accelerator addresses it", "Status / authority responsibility"], [
    ...
    ["Reproducible control evidence (CA/SA)", "Reproducible AWS CDK infrastructure-as-code, a 189-test automated suite gating every change in CI, and captured clean-account validation runs (the full governed workflow live end-to-end; the Cedar authorization gateway deployed as code in enforcement mode; and the complete hardening posture — private networking with a HUD-only egress allowlist, customer-managed KMS, MFA-required identity, concurrency/replay proofs, and a PII telemetry canary) committed in the repo's evidence/ folder.", [{ text: "Authority: ", bold: true }, "the StateRAMP / NIST 800-53 control set, the SSP, and the authorization to operate."]],
  ], [2500, 4240, 3700]),
