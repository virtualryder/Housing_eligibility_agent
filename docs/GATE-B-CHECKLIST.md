...
| B4 | **PII telemetry-leak canary** — unique fake-PII marker run through the live pipeline, then swept across CloudWatch Logs, X-Ray, Step Functions history, DLQs; `--strict` = every destination clean | `scripts/pii_canary.py` | offline verdict tests + **LIVE: PASS — 0 hits in Logs/X-Ray/DLQs; SFN-history finding quantified (87) → pass-by-reference remediation tracked** | ✅ **live-run PASS**; strict-pass pending remediation |
