...
- **Immutable WORM audit:** an append-only, tamper-evident record (append-only DynamoDB + S3 Object Lock) capturing `INTENT → COMMITTED` for the due-process evidence trail.
...
7. **Manifest + validate** — the whole agent is one manifest; deploy; end-to-end run (Cedar allow/deny, live HUD lookup, masking, eligibility, real Bedrock notice, immutable audit) + negative tests; teardown.
...
- **Privacy Act / PII safeguarding** (applicant SSNs, income records) → fail-closed masking + least-privilege Cedar + immutable audit + encryption.
