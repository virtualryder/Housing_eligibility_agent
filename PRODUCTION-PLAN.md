...
| # | Item | Status |
|---|---|---|
| A1 | **Full AgentCore/Gateway/Cedar path in CDK** | ✅ **built + assertion-proven** — `GatewayStack` custom-resource provider executes the proven engine sequence (policy engine → MCP gateway CUSTOM_JWT → SSM discovery → 9 targets built AT SYNTH from the manifest → 7 Cedar policies w/ gateway-ARN injection → ENFORCE), reverses on delete; gateway role invokes exact Lambda ARNs only; assertions prove every manifest tool attached, every policy loaded, no credential field in any gateway schema, ENFORCE asserted. **Live exercise of the provider = the GA-4 run** |
| A2 | **Secrets Manager** for signing secret + HUD token (no plaintext in context/CFN/env; least-privilege grants; CloudTrail-visible reads; rotation via version) | ✅ **DONE incl. GA-2 key separation** — `SigningSecretDeid` + `SigningSecretHud` + `HudTokenSecret` in CDK; ARN resolution cached + fail-closed; **domain-scoped keys**: mask_pii signs sanitized refs with the deid key ONLY, the lookup signs HUD provenance with the HUD key ONLY, verifiers resolve their own domain's key, and IAM splits the grants (lookup can't read the deid key, masker can't read the HUD key) so cross-domain forgery is an IAM impossibility. A configured-but-unreadable domain key fails closed (never degrades to a shared key). Tests: cross-domain forgery matrix + CDK split-grant assertions (`test_secrets_path.py`, `test_cdk_stacks.py`) |
...
| A4 | **Live end-to-end validation** | ✅ **CAPTURED** (evidence/GA-4-LIVE-HAPPY-PATH.md + addendum) — full governed pipeline SUCCEEDED live (HUD + Comprehend + SM secrets + Bedrock + WORM + SoD + exactly-once finalize) AND the IaC gateway attachment reached CREATE_COMPLETE in **ENFORCE** (9 targets, 7 Cedar policies, CUSTOM_JWT). 7 live defects found+fixed by the runs. Cedar allow/deny through the gateway with real user JWTs = Gate-B IdP work |
| A5 | **Idempotent finalization + duplicate/replay protections** | ✅ **DONE** — exactly-once FINAL#<case> conditional marker (retries + different-approver paths return the ORIGINAL submission, one COMMITTED record); duplicate-submission conditional register (fail-closed, first task token preserved); approval bound to `States.Hash` of the assessment the approver saw; `tests/test_exactly_once.py` |
| A6 | **Minimum CloudWatch dashboards + alarms (CDK ObservabilityStack)** | ✅ **DONE** — workflow failed/timed-out/throttled + governance-Lambda error alarms → SNS ops topic; operations dashboard; CDK-asserted. Custom security metrics (forged-ref counts etc.) extend under Gate B |
| A7 | **DEPLOYMENT-GUIDE.md (CDK-only)** + validation script | ✅ **DONE** — guide + `scripts/validate_deployment.py` emitting the PASS/FAIL JSON verdict + troubleshooting + upgrade/rollback/uninstall |
| A8 | **Formal GitHub Release** for the validated tag (source archive, SBOM, checksums, validation evidence, known limitations, changelog) — a tag alone is not a release package | ✅ **DONE** — `v0.9.0-pilot-rc1` published; superseded by **`v0.9.1-pilot-rc2`** packaging the complete GA-4 evidence (happy path + IaC gateway ENFORCE capture) with README/docs/GTM refreshed to match |
| A9 | **Cedar policy property tests + new-tool CI gate** | ✅ **DONE** — `tests/authz_expectations.yaml` contract + `test_cedar_properties.py`: single group-gated permit (deny-by-default), forbids name every human-only action (unconditional) and every masking gate (`unless deidentified`), tools hidden from workflow steps, and any NEW tool fails CI until its posture is declared |
| A10 | **Supply-chain CI** | ◐ mostly done — CodeQL workflow, blocking bandit (medium+), CODEOWNERS, SECURITY.md shipped. **Owner clicks needed in GitHub settings:** enable secret scanning + push protection, protect `main` (required reviews). cdk-nag staged next |
...
| B4 | PII telemetry-leak canary — marked case swept across Logs/X-Ray/SFN history/DLQs (`scripts/pii_canary.py`) | ✅ **STRICT LIVE PASS (Review-3 R3-2)** — pass-by-reference orchestration shipped; 0 hits in EVERY destination incl. SFN history; CDK assertion pins zero-content state |
...
- *(cycle 4)* **P0-11 COMPLETE — live clean-account validation captured (evidence/P0-11-VALIDATION-RUN.md).**
  ...
  fixed + regression-tested (suite 98/98 <!-- count-gate:historical --> at that point). Full teardown verified: stacks deleted, RETAIN'd sandbox
