...
Why this exists. Before this gate, README.md and VALIDATED_RELEASE.md both quoted **161** while the
suite had grown past it, and PRODUCTION-PLAN.md carried a trail of older counts (97/97, 98/98) with
nothing distinguishing "what was true during that cycle" from "what is true now". The cause was
structural: `test_release_consistency.py` gated tag references but nothing asserted a count, so every
suite change silently invalidated the docs.

A reviewer who spot-checks one number and finds it wrong stops trusting the control claims too. In a
housing-assistance review - where the due-process and income-determination claims are the product -
that is expensive. So the count is now machine-enforced.
"""
import pathlib
import re
...
COUNTED_DOCS = [
    ...
    "PILOT-SCOPE.md", "PRODUCTION-PLAN.md", "DEPLOYMENT-GUIDE.md",
    "docs/GATE-B-CHECKLIST.md", "evidence/GATE-B-VALIDATION.md",
    "evidence/GA-4-LIVE-HAPPY-PATH.md", "evidence/P0-11-VALIDATION-RUN.md",
]
...
def _collected_count():
    ...
    env = dict(os.environ, HOU_DOC_COUNT_GATE="1")
    out = subprocess.run(
...
@pytest.mark.skipif(__import__("os").environ.get("HOU_DOC_COUNT_GATE") == "1",
    ...
    for rel in COUNTED_DOCS:
        ...
        for pat in COUNT_PATTERNS:
            for m in pat.finditer(text):
                ...
                # "at the time of this run" scopes the WHOLE line (it is a past-run record).
                # The inline marker scopes ONE number: it must sit within 40 chars AFTER that
                # number. Line-scoping the marker was the drift: a line that records a historical
                # count AND the current one - "the suite is 395 tests; v0.3.0 stood at 154
                # <!-- count-gate:historical -->" - had BOTH numbers exempted, so the current
                # count could rot behind a green gate. Ported from the benefits pack (2026-09-06).
