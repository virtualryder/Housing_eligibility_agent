...
right behaviour on a laptop without the CDK libs installed - but the CDK libs were pinned only as an unpinned range inline in the workflow (`aws-cdk-lib>=2.150`), so a verifier could resolve a different CDK than the one validated, so the 24 CDK security
assertions could drift while the build still reported green.
...
exact-ARN least-privilege IAM, and the "no applicant PII in Step Functions state" pass-by-reference
