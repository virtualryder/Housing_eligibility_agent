...
> validated release tag [`v0.11.0`](https://github.com/virtualryder/Housing_eligibility_agent/releases/tag/v0.11.0)**
...
[`PRODUCTION-PLAN.md`](PRODUCTION-PLAN.md). The following are **built and offline-proven (219 tests
...
```bash
git checkout v0.11.0                      # deploy a VALIDATED RELEASE TAG, never main
...
**This pack had no security workflow at all until 2026-09-08** — bandit, detect-secrets, checkov and
trivy had never run against it, and nothing detected that for a month because the parity check
compares controls, not the CI that scans for them (L47). The first run found 14 blocking bandit
findings, a two-month-stale base image, and 67 checkov findings now baselined as the **starting
point** of a burn-down, not as a resolution.
