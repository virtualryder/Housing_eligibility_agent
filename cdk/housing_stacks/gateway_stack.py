...
def _targets_from_manifest(compute):
    ...
    m = yaml.safe_load((REPO / "agents" / "housing-assistance" / "manifest.yaml").read_text(encoding="utf-8"))
    fn_by_target = {
        "intake-application": compute.intake, "lookup-income-limit": compute.lookup,
        "mask-pii": compute.mask, "assess-eligibility": compute.assess,
        "recertify": compute.recertify, "detect-overpayment": compute.overpayment,
        "hou-core": compute.core, "write-audit": compute.write_audit,
...
class GatewayStack(cdk.Stack):
    def __init__(self, scope: Construct, cid: str, *, prefix: str, compute, identity, **kw):
        ...
        ssm_param = f"/{prefix}-housing/gateway-url"
        ...
        attachment = cdk.CustomResource(
            ...
            properties={
                ...
                "PoliciesJson": json.dumps(_policies(prefix)),
                "Enforcement": "ENFORCE",
            })
