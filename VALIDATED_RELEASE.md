...
| Field | Value |
|---|---|
| Tag | `v0.11.0` (immutable; **2026-09-05**) — supersedes `v0.10.0` (2026-08-03, governed-core 1.4.0) and the 2026-07-24 EP1/Gate-B tags. Single source of truth: the repo-root `RELEASE` file, enforced by `tests/test_release_consistency.py` (tag drift across docs fails CI) **`v0.11.0` (2026-09-05) supersedes `v0.10.0 / v0.9.6`**: the same tree with `lib/` relocked at the pinned **governed-core 1.10.1** (fault-semantics batch — durable-evidence gating, un-strandable approvals, authoritative Cedar context — plus deepened PII/PHI detection); **offline-gated** (all pass locally except one CI-only gate; 187 collected <!-- count-gate:historical -->). The last LIVE gate on this pack ran on the EP1 Gate-B run of 2026-07-24 (tag `v0.9.6`); `v0.10.0` (2026-08-03) pinned governed-core 1.4.0; the 1.10.1 re-pin is `v0.11.0`'s change, offline only. **No AgentCore-era live gate (111 / multi-tenant / kill switch / budget / perimeter / runtime) has ever run on this pack** — those controls are inherited from governed-core and are IaC/offline-asserted here, not live-proven; a live re-gate on 1.10.1 is the next milestone for this pack (third external review, REL-2). |
| Commit SHA | the commit carrying tag `v0.11.0` (`git rev-list -n1 v0.11.0`) |
| Test count at tag | **161 tests** <!-- count-gate:historical --> at the moment `v0.9.6` was cut (134 offline + 24 CDK assertions + 3 CI-completeness gates) — a record of that tag, not a claim about the current tree |
| Test count on current main | **219 tests**, run in CI on every push. 206 pass locally; 1 CI-completeness gate runs only in CI. Machine-enforced by `tests/test_doc_counts.py`. Tag `v0.11.0` was cut when the suite stood at 187 <!-- count-gate:historical --> and therefore does NOT match this count: main has moved ahead of the tag. |
...
| Defect found + fixed by the run | `p011-validation-1` FAILED `States.Runtime` (brittle guard JSONPath on a source-down lookup) → guard now judges the whole lookup output; regression test added; suite **98/98** <!-- count-gate:historical --> at that point |
